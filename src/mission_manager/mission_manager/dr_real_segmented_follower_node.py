...
class DrRealSegmentedFollower(Node):
    ...
    def __init__(self):
        ...
        # Mode 5 LiDAR avoidance reference path.
        self.declare_parameter(
            "avoidance_reference_topic",
            "/avoidance/route/reference_path")
        self.declare_parameter(
            "avoidance_reference_segment", "COMMON_1")
        self.declare_parameter(
            "avoidance_reference_mode", 5)
        self.declare_parameter(
            "avoidance_reference_post_points", 10)

        # Vehicle command topics
        self.declare_parameter("gps_drive_topic", "/cmd_drive")
        self.declare_parameter("gps_wheel_topic", "/cmd_wheel")
        self.declare_parameter("lidar_drive_topic", "/mission/lidar_drive_request")
        self.declare_parameter("lidar_wheel_topic", "/mission/lidar_wheel_request")
        ...
        self.avoidance_reference_topic = str(
            gp("avoidance_reference_topic"))
        self.avoidance_reference_segment = str(
            gp("avoidance_reference_segment")).strip()
        self.avoidance_reference_mode = int(
            gp("avoidance_reference_mode"))
        self.avoidance_reference_post_points = max(
            0, int(gp("avoidance_reference_post_points")))
        ...
        self.avoidance_reference_pub = self.create_publisher(
            NavPath,
            self.avoidance_reference_topic,
            latched)
    ...
    def _align_all_segments(self):
        ...
        self._publish_avoidance_reference()

    def _publish_avoidance_reference(self):
        """Publish aligned Mode-5 reference path for LiDAR avoidance."""
        pts = self.segments.get(
            self.avoidance_reference_segment, [])

        indices = [
            i for i, point in enumerate(pts)
            if int(point["mode"]) == self.avoidance_reference_mode
        ]

        if not indices:
            self.get_logger().warning(
                "No avoidance reference points found: "
                f"segment={self.avoidance_reference_segment} "
                f"mode={self.avoidance_reference_mode}")
            return

        first = indices[0]
        last = indices[-1]

        # Do not silently connect disjoint Mode-5 blocks.
        expected = list(range(first, last + 1))
        if indices != expected:
            self.get_logger().error(
                "Mode-5 reference is not contiguous; "
                "avoidance reference not published")
            return

        end = min(
            len(pts),
            last + 1 + self.avoidance_reference_post_points)

        selected = pts[first:end]

        msg = NavPath()
        msg.header.frame_id = self.current_odom_frame
        ...
        self.avoidance_reference_pub.publish(msg)

        self.get_logger().info(
            "Mode-5 avoidance reference published: "
            f"segment={self.avoidance_reference_segment} "
            f"indices={first}..{end-1} "
    ...
    def _on_odom(self, msg: Odometry):
        ...
        self.current_odom_frame = (
            str(msg.header.frame_id).strip() or "odom")
    ...
    def _follow_current_segment(self):
        ...
        self._publish_command(owner, drive, int(round(steer_deg)))
        ...
        self._status(
            ...
            f"wheel={int(round(steer_deg))} owner={owner}")
    ...
    def _maybe_enter_stopline_wait(self, pts, old_cursor, new_cursor):
        ...
        for event_idx in stop_indices:
            ...
            # START_A/START_B mode 2 STOP_LINE:
            # stop once, then automatically continue after intersection_wait_sec
            # (3.0 s by current config). It does not wait for camera GO.
            if mode == 2:
                self._enter_wait("MODE2_TIMED_STOP", key)
                return True
    ...
    def _process_wait(self):
        ...
        if self.wait_state == "MODE2_TIMED_STOP":
            if elapsed >= self.intersection_wait_sec:
                self._finish_wait("MODE2_TIMEOUT_RELEASE")
                return
            self._status(f"WAIT_MODE2_TIMED_STOP {elapsed:.2f}s")
            return
        ...
        if self.wait_state == "INTERSECTION":
            ...
            if elapsed >= self.intersection_wait_sec:
                ...
                # MCU의 별도 intersection policy는 mode 4/6에만 있다.
                if mode in (4, 6):
            ...
            self._status(f"WAIT_INTERSECTION {elapsed:.2f}s")
            return
    ...
    def _publish_owner_stop(self, owner):
        if owner == "lidar":
    ...
    def _publish_all_stop(self):
        self._publish_owner_stop("gps")
        self._publish_owner_stop("lidar")
